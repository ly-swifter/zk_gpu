...
pub mod tests {
    ...
    fn test_threadpook()
    {
        ...
        let rets : Vec<Waiter<_>> = (0..=3).map( |i| {
            worker.compute(move || computing(i))

        }).collect();
        let rets: Vec<usize> = rets.iter().map(|ret| {
            ret.wait()
        }).collect();
        log::info!("Final result of test is {:?}", rets);
    }

    fn computing(init_value: usize) -> usize
    {
        log::info!("start computing for initial value {:?}", init_value);
        ...
        log::info!("finish computing for initial value {:?}", init_value);
